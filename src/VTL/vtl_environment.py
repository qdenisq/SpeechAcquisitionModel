...
class VTLEnv(object):
    def __init__(self, lib_path, speaker_fname, timestep=10, max_episode_duration=1000, img_width=400, img_height=400):
        ...
        self.action_bound = list(zip(-abs(np.subtract(self.tract_param_min, self.tract_param_max)),
                                     abs(np.subtract(self.tract_param_max, self.tract_param_min))))
        self.action_bound.extend(zip(-abs(np.subtract(self.glottis_param_min, self.glottis_param_max)),
                                     abs(np.subtract(self.glottis_param_max, self.glottis_param_min))))
