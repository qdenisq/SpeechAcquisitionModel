...
class ImportanceSamplingReplayBuffer(ReplayBuffer):
    def __init__(self, buffer_size, random_seed=0):
        super(ImportanceSamplingReplayBuffer, self).__init__(buffer_size, random_seed)

    def sample_batch(self, batch_size):
        batch = []

        sample_prob = [x*x / self.count for x in np.arange(0, self.count)]
        sample_prob /= np.sum(sample_prob)



        if self.count < batch_size:
            idx = np.random.choice(range(self.count), p=sample_prob, size=self.count)
            batch = [self.buffer[x] for x in idx]
            # batch = random.sample(self.buffer, self.count)
        else:
            idx = np.random.choice(range(self.count), p=sample_prob, size=batch_size)
            batch = [self.buffer[x] for x in idx]
            # batch = random.sample(self.buffer, batch_size)
    ...
    for i in range(num_episodes):
        ...
        if replay_buffer.size() > minibatch_size:
            ...
            for _ in range(n_train_steps):
                ...
                ##############################################################
                policy_loss_out = 0.
                # train policy if md is accurate enough
                if md_loss < md_loss_thresh:
...
def main():
    ...
    replay_buffer = ImportanceSamplingReplayBuffer(3000)
