...
class LSTMCriticNetwork(object):
    ...
    def __init__(self, name, model_settings):
        ...
        with tf.variable_scope(self.name + '_critic'):
            self.state, self.next_state, self.sequence_length_placeholder, self.out = self.create_critic_network()
            self.network_params = tf.trainable_variables(scope=self.name + '_critic')

        # Target Network
        with tf.variable_scope(self.name + '_target_critic'):
            self.target_state, self.target_next_state, self.target_sequence_length_placeholder, self.target_out = self.create_critic_network()
        ...
        self.next_state_grads = tf.gradients(self.out, self.next_state)
    ...
    def create_critic_network(self):
        ...
        fc_layer = TimeDistributed(Dense(50, kernel_initializer=RandomUniform(minval=-0.003, maxval=0.003)))(lstm_outputs)
    ...
    def train(self, states, next_states, predicted_q_values, sequence_length):
    ...
    def predict_target(self, states, next_states, sequence_length):
    ...
    def next_state_gradients(self, states, next_states, sequence_length):
...
class LSTMWorldModelerNetwork(object):
    ...
    def __init__(self, name, model_settings):
        ...
        # Create the critic network
        with tf.variable_scope(self.name + '_world_modeler'):
            self.state, self.action, self.sequence_length_placeholder, self.out, self.scaled_out = self.create_world_modeler_network()
            self.network_params = tf.trainable_variables(scope=self.name + '_world_modeler')

        # Target Network
        with tf.variable_scope(self.name + '_target_world_modeler'):
            self.target_state, self.target_action, self.target_sequence_length_placeholder, self.target_out, self.target_scaled_out = self.create_world_modeler_network()
            self.target_network_params = tf.trainable_variables(scope=self.name + '_target_world_modeler')
        ...
        # Network target (y_i)
        self.actual_next_state = tf.placeholder(tf.float32, [None, None, self.s_dim])

        # Define loss and optimization Op
        # self.loss = tf.losses.mean_squared_error(self.predicted_q_value, self.out)
        self.loss = tflearn.mean_square(self.scaled_out, self.actual_next_state)
        self.optimize = tf.train.AdamOptimizer(
            self.learning_rate).minimize(self.loss)
        ...
        # w.r.t. that action. Each output is independent of all
        # actions except for one.
        self.next_state_grads = tf.placeholder(tf.float32, [None, None, self.s_dim])
        self.action_grads = tf.gradients(self.scaled_out, self.action, self.next_state_grads)
    ...
    def create_world_modeler_network(self):
        ...
        # final dense layer
        w_init = RandomUniform(minval=-0.003, maxval=0.003)
        last_layer = TimeDistributed(Dense(self.s_dim, activation='tanh', kernel_initializer=w_init))
    ...
    def train(self, states, actions, actual_next_state, sequence_length):
        return self.sess.run([self.out, self.optimize, self.loss], feed_dict={
            self.state: states,
            self.action: actions,
            self.actual_next_state: actual_next_state,
            self.sequence_length_placeholder: sequence_length
        })

    def predict(self, states, actions, sequence_length):
    ...
    def action_gradients(self, states, actions, sequence_length, next_state_grads):
        return self.sess.run(self.action_grads, feed_dict={
            self.state: states,
            self.action: actions,
            self.next_state_grads: next_state_grads,
            self.sequence_length_placeholder: sequence_length
        })
...
def get_default_world_modeler_settings():
    settings = {
        'world_modeler_learning_rate': 0.001,
        'world_modeler_tau': 0.01,
        'world_modeler_lstm_num_cells': 100,
        'world_modeler_batch_size': 64
    }
    return settings
...
def build_summaries():
    ...
    wm_loss = tf.Variable(0.)
    tf.summary.scalar("World modeler loss", wm_loss)
    ...
    summary_vars = [episode_reward, episode_ave_max_q, actor_ep_loss, critic_loss, wm_loss, avg_action, act_grads, actor_grads, actor_activations]
...
def train(sess, env, settings, args, actor, critic, world_modeler, actor_noise):
    # Set up summary Ops
    ...
    world_modeler.init_target_network()
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            if replay_buffer.size() > minibatch_size:
                ...
                # update world_modeler
                _, _, wm_loss = world_modeler.train(s_batch, a_batch, s2_batch, [episode_length] * minibatch_size)

                # Calculate targets
                target_actions, _ = actor.predict_target(s2_batch, [episode_length] * minibatch_size, minibatch_size)
                target_next_states = world_modeler.predict_target(s2_batch, target_actions, [episode_length] * minibatch_size)
                target_q = critic.predict_target(s2_batch, target_next_states, [episode_length] * minibatch_size)
                ...
                predicted_q_value, _, critic_loss = critic.train(
                    s_batch, s2_batch, y_i, [episode_length] * minibatch_size)
                ep_ave_max_q += np.amax(predicted_q_value)
                # Update the actor policy using the sampled gradient
                a_outs, _ = actor.predict(s_batch, [episode_length] * minibatch_size, minibatch_size)
                next_state_outs = world_modeler.predict(s_batch, a_outs, [episode_length] * minibatch_size)
                next_state_grads = critic.next_state_gradients(s_batch, next_state_outs, [episode_length] * minibatch_size)
                action_grads = world_modeler.action_gradients(s_batch, a_batch, [episode_length] * minibatch_size, next_state_grads[0])
            ...
            if terminal:
                summary_str = sess.run(summary_ops, feed_dict={
                    ...
                    summary_vars[4]: wm_loss,
                    summary_vars[5]: action_avg / j,
                    summary_vars[6]: action_grads,
                    summary_vars[7]: actor_grads[0],
                    summary_vars[8]: a_outs
                ...
                print('| Reward: {:d} | Episode: {:d} | Qmax: {:.4f} | Critic Loss: {:.4f}| WM Loss: {:.4f}'.format(int(ep_reward),
                                                                                                                       i,
...
def main(args):
    with tf.Session() as sess:
        ...
        train(sess, env, settings, args, actor, critic, world_modeler, actor_noise)
